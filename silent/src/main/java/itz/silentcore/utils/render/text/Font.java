package itz.silentcore.utils.render.text;
...
import itz.silentcore.utils.render.text.FontData.AtlasData;
import itz.silentcore.utils.render.text.FontData.GlyphData;
import itz.silentcore.utils.render.text.FontData.MetricsData;
...
public final class Font {
	...
	private Font(String name, AbstractTexture texture, AtlasData atlas, MetricsData metrics, Map<Integer, Glyph> glyphs, Map<Integer, Map<Integer, Float>> kernings) {
		this.name = name;
	...
	public void applyGlyphs(Matrix4f matrix, VertexConsumer consumer, String text, float size, float thickness, float spacing, float x, float y, float z, int color) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			Glyph glyph = this.glyphs.get(_char);
    ...
    public static Font.Builder builder() {
		return new Builder();
	}
	...
	public static class Builder {
		...
		public Font.Builder name(String name) {
			this.name = name;
			return this;
		}
		
		public Font.Builder data(String dataFileName) {
			this.dataIdentifer = Identifier.of("silentcore", "fonts/" + dataFileName + ".json");
			return this;
		}
		
		public Font.Builder atlas(String atlasFileName) {
			this.atlasIdentifier = Identifier.of("silentcore", "fonts/" + atlasFileName + ".png");
			return this;
		}
		...
		public Font build() {
			...
			Map<Integer, Glyph> glyphs = data.glyphs().stream()
					.collect(Collectors.<GlyphData, Integer, Glyph>toMap(
							(glyphData) -> glyphData.unicode(),
							(glyphData) -> new Glyph(glyphData, aWidth, aHeight)
					));
