    ...
    public WebRendererInstance create(WebRendererConfig config) {
        return new WebRendererInstance() {
            ...
            public void render(DrawContext drawContext, int x, int y, int width, int height, float tickDelta) {
                ...
                drawContext.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, Text.translatable("xwebrenderer.web.no_engine"), x + 6, y + 6, 0xFFDC143C);
                drawContext.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, Text.literal("Configured URL: " + config.initialUrl()), x + 6, y + 18, 0xFFD0D0D0);
            }
